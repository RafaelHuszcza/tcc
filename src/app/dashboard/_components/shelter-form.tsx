...
export function ShelterForm({ method, defaultValues }: ShelterForm) {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues:  {
      ...
      currentOccupancy: defaultValues?.currentOccupancy ? defaultValues.currentOccupancy : 0,
      description: defaultValues?.description ? defaultValues.description : '',
      phone: defaultValues?.phone ? defaultValues.phone : '',
      serviceHours: defaultValues?.serviceHours ? defaultValues.serviceHours : '',
      status: defaultValues?.status ? defaultValues.status : ShelterStatus.ACTIVE,
  ...
  return (
    <main className='items-center justify-center flex w-full flex-1'>
    <Card className="w-full max-w-4xl">
    <CardHeader>
      <CardTitle>{method === 'POST' ? 'Adicionar novo Abrigo' : 'Editar Abrigo'}</CardTitle>
    </CardHeader>
    <CardContent>
      <Form {...form}>
        <form onSubmit={onSubmit} className="flex flex-wrap gap-4  justify-center">
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="min-w-60">
                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  ...
                  <SelectContent>
                    <SelectItem value={ShelterStatus.ACTIVE}>Ativo</SelectItem>
                    <SelectItem value={ShelterStatus.INACTIVE}>Inativo</SelectItem>
                  </SelectContent>
                </Select>
                <FormMessage />
              </FormItem>
          ...
          <Button type="submit" className='min-w-60' disabled={form.formState.isSubmitting}>
